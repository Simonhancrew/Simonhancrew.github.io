...
很多女性在使用口服避孕药，OCP 会抑制内源性激素波动，因此上述周期化建议可能需要调整。

**OCP 对激素环境的影响**：

- 抑制 LH/FSH 分泌，阻止排卵，内源性雌激素和孕酮水平显著降低
- 外源性合成激素（炔雌醇+孕激素）提供稳定但非生理性的激素水平
- 月经周期的自然激素波动被消除，"月经"实际是撤退性出血

**对训练表现的影响**[^16]：

- 荟萃分析显示 OCP 对力量和有氧表现的影响**很小且不一致**
- 部分研究显示 OCP 使用者最大力量可能轻度降低（~2-4%），但临床意义不大
- OCP 可能轻度增加 SHBG，降低游离睾酮，理论上不利于增肌，但实际效应量很小

**对增肌的影响**：

- 目前证据不支持"OCP 显著阻碍增肌"的结论
- 少数研究显示 OCP 使用者瘦体重增长可能略慢，但差异不具临床意义
- 不需要因为增肌目的停用 OCP

**对周期化训练的影响**：

- 使用 OCP 的女性没有自然的激素波动，上述"滤泡期冲量、黄体期降量"的建议**不适用**
- 可以按照稳定的训练计划执行，不需要根据"周期"调整
- 停药周（出血周）如果有不适，可以适当降低强度，但不是激素驱动的

**实践建议**：

- 使用 OCP 的女性按照常规线性/波动周期化训练即可
- 不需要模拟月经周期调整训练
- 如果考虑停药，注意前几个月激素波动可能较大，训练表现可能暂时不稳定

[^16]: Elliott-Sale KJ, et al. (2020). The effects of oral contraceptives on exercise performance in women: a systematic review and meta-analysis. *Sports Medicine*, 50(10):1785-1812.
...
- 一项小样本研究（Roberts 2020）初步显示：这种分阶段安排比持续恒定热量，在相同时间内，减去更多脂肪，同时保留更多肌肉[^13]。但目前证据有限（样本量小，单一研究），需要更多研究验证
