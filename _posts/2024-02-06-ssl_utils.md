...
BIO(rbio, wbio)和socket读写之间的关系就是下面这样的，在tcp上面多了一个tls的组件。

ondata之类的input事件，先考虑shake hand，你其实先要塞给tls，你读到了一个buffer，通过BIO_write塞到readio里，然后进入一个ssl连接对象读取的阶段。在shakehands的时候，可能ssl会主动的想要写，你也需要回调到上面去

在send之类的事件的时候，你需要加密你的buffer，先call SSL_write，然后成功加密，从wbio里读到这个数据，继续告诉上层，发送的数据ready了，可以继续发了。
